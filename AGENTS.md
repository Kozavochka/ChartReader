...
- Для line-only обучения критичен правильный маппинг категорий:
  `self._cat_ids = [2]` (COCO id линии) -> `_coco_to_class_map = {2: 0}`.
  В `_extract_data` класс `category == 0` должен обрабатываться как **line**
  (полилинии), иначе bbox-логика для bar исказит точки и ухудшит таргеты/лосс.
